...
import { getCuotasPendientesFamilia, type ICuotaPendienteItem } from "../api/getCuotasPendientesFamilia";
...
export default function PagoFamiliarDialog({
  ...
  const [cuotas, setCuotas] = useState<ICuotaPendienteItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchCuotas = async () => {
      ...
      const { data } = await getCuotasPendientesFamilia(representante.id);
      const list = data ?? [];
      setCuotas(list);

      const initialSelected: Record<string, number> = {};
      list.forEach((c) => {
        initialSelected[c.cuotaId] = c.saldo;
      });
      setSelectedCuotas(initialSelected);
      ...
      <AlertDialogContent className="w-full max-w-2xl sm:max-w-3xl max-h-[90vh] overflow-y-auto p-6 sm:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <AlertDialogFooter className="pt-2 gap-3">
            <AlertDialogCancel type="button" disabled={registrarMutation.isPending} className="h-10 px-5">
              Cancelar
            </AlertDialogCancel>
            <Button
              type="submit"
              disabled={registrarMutation.isPending || totalPagar <= 0}
              className="h-10 px-6 font-semibold"
            >
              {registrarMutation.isPending && <Spinner className="size-4 mr-2" />}
