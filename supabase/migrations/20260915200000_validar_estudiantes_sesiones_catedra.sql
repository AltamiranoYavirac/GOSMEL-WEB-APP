...
begin
  ...
  if not exists (
    select 1 from public.inscripciones i
     where i.catedra_id = p_catedra_id and i.estado in ('activa', 'pendiente')
  ) then
    raise exception 'La cátedra no tiene estudiantes: matricula al menos uno antes de generar el calendario';
  end if;
